...
var someOptionalString: String? = "optional12" // Can be nil
// T? is shorthand for Optional<T> — ? is a postfix operator (syntax candy)
let someOptionalString2: Optional<String> = "nil"
let someOptionalString3 = String?.some("optional1234") // same as the first one
let someOptionalString4 = String?.some("none") //nil
...
if let someNonOptionalStringConstant = someOptionalString {
    ...
    if !someNonOptionalStringConstant.hasPrefix("ok") {
        print("does not have the prefix")
    }
}

//if-var is allowed too!
if var someNonNonOptionalString: String? = someOptionalString {
    someNonNonOptionalString = "actually optional AND mutable"
    print(someNonNonOptionalString)
}
...
for (person) in immutableOccupations {
    print("\(person.key)'s job is\(person.value), and wtf is\(person.self.self)")
}
for i in 1...5 {
...
for i in stride(from: 3, to: 52, by: 8){
    print(i)
}
...
print(sayHello(to: "World", onDay: "Sunday"))
...
func printParameters(_ r: Int, _ o: Int = 10) {
...
print(price3 == pricesTuple2.highestPrice)
...
print("No newline","pes", terminator: "")
print("!")

// MARK: - Closures

var numbers = [1, 2, 6]
print("No newline","pes",numbers, terminator: "\0")
...
foo <-> bar
print("foo is \(foo), bar is \(bar)") // "foo is 20, bar is 10"
...
func testTryStuff() {
    ...
    } catch MyError.reallyBadValue(let msg) {
